...
use crate::input::{
    self, ActionContext as _, DeferredKeys, Execute, FONT_SIZE_STEP,
};
...
impl<'a, N: Notify + 'a, T: EventListener> input::ActionContext<T> for ActionContext<'a, N, T> {
    ...
    fn write_to_pty<B: Into<Cow<'static, [u8]>>>(&mut self, val: B) {
        if let Some(bytes) = self.deferred_keys.write_or_queue(self.active_tab_id, val.into()) {
    ...
    fn defer_key_actions(
        ...
        if let Some(input) = self.deferred_keys.push(id, actions, self.active_tab_id, input) {
            self.notifier.notify(input);
        }

        // One timer per window covers the active deferred keys; queued keys start their timeout
        // only once their input has been forwarded.
        let window_id = self.display.window.id();
        let timer_id = TimerId::new(Topic::DeferredKey, window_id);
        if !self.scheduler.scheduled(timer_id) {
            let event = Event::new(EventType::DeferredKeyTimeout, window_id);
            let now = Instant::now();
            let deadline = self.deferred_keys.next_deadline().unwrap_or(now);
...
impl input::Processor<EventProxy, ActionContext<'_, Notifier, EventProxy>> {
    /// Forward queued input in order until another deferred key requires an application reply.
    fn resume_deferred_input(&mut self, tab_id: Option<usize>, now: Instant) {
        for input in self.ctx.deferred_keys.resume(tab_id, now) {
            self.ctx.notifier.notify(input);
        }
    }

    /// Point the window's single deferred-key timer at the oldest active report deadline.
    fn reschedule_deferred_key_timeout(&mut self, now: Instant) {
            ...
            WinitEvent::UserEvent(Event { payload, tab_id, .. }) => match payload {
                ...
                EventType::DeferredKeyTimeout => {
                    ...
                    while let Some(deferred) = self.ctx.deferred_keys.pop_expired(now) {
                        let tab_id = deferred.tab_id;
                        if tab_id == self.ctx.active_tab_id {
                            for action in deferred.actions {
                                action.execute(&mut self.ctx);
                            }
                            self.resume_deferred_input(tab_id, now);
                        } else {
                            self.ctx.deferred_keys.discard(tab_id);
                        }
                    }

                    self.reschedule_deferred_key_timeout(now);
                },
                ...
                EventType::Terminal(event) => match event {
                    ...
                    TerminalEvent::KeyEventHandled(id, handled) => {
                        ...
                        if let Some(deferred) = self.ctx.deferred_keys.take(id, source_tab_id) {
                            ...
                            if tab_id == self.ctx.active_tab_id {
                                if handled {
                                    // The application consumed the forwarded key as terminal
                                    // input, which bypassed `key_input`'s normal input handling.
                                ...
                                self.ctx.deferred_keys.discard(tab_id);
                                self.reschedule_deferred_key_timeout(Instant::now());
                            }
                        }
                    },
